...
public class GajiController {
    ...
    public void cariKaryawan(javax.swing.JTextField ktp){
        if (!ktp.getText().equals("")){
            if (karyawan.baca(ktp.getText())){
                    ...
                    if (listGaji.length>0){
                        for (int i=0; i<listGaji.length;i++){
                            if (!((String)listGaji[i][0]).equals("")){
                                ...
                                FormUtama.formGaji.setTambahGaji(new Object[]{listGaji[i][0], namaPekerjaan, listGaji[i][3], listGaji[i][1], listGaji[i][2]});
                                jumlahGaji++;
                            }
                        }
                    }
                ...
                if (jumlahGaji==0) {
                    FormUtama.formGaji.setTambahGaji(new Object[]{"", "", "0", "0", "0"});
                }
            } else {
    ...
    public void simpan(javax.swing.JTextField ktp, javax.swing.JTable gajiTable){
        if (!ktp.getText().equals("")){
            ...
            if (FormUtama.formGaji.getGajiTable().isEditing()) {
                FormUtama.formGaji.getGajiTable().getCellEditor().stopCellEditing();
            }
            
            Object[][] listGaji = new Object[gajiTable.getRowCount()][4];
            
            for (int i=0; i<gajiTable.getRowCount();i++){
                listGaji[i][0] = (gajiTable.getValueAt(i, 0) != null) ? gajiTable.getValueAt(i, 0).toString().trim() : "";
                listGaji[i][1] = gajiTable.getValueAt(i, 3); // Gaji kotor
                listGaji[i][2] = gajiTable.getValueAt(i, 4); // Tunjangan
                listGaji[i][3] = gajiTable.getValueAt(i, 2); // Gaji bersih
            }
            
            gaji.setListGaji(listGaji);
            
            if (gaji.simpan()){
                JOptionPane.showMessageDialog(null, "Data gaji berhasil disimpan!", "Sukses", JOptionPane.INFORMATION_MESSAGE);
