...
public class FormGaji extends javax.swing.JInternalFrame {
    ...
    public javax.swing.JTable getGajiTable() {
        return gajiTable;
    }
    ...
    public void setTambahGaji(Object[] gajiPekerjaan) {
        int rowCount = gajiDefaultTableModel.getRowCount();
        if (rowCount > 0) {
            // Check if last row is empty
            Object firstCell = gajiTable.getValueAt(rowCount - 1, 0);
            if (firstCell == null || firstCell.toString().isEmpty()) {
                // Replace last empty row
                gajiDefaultTableModel.removeRow(rowCount - 1);
            }
        }
